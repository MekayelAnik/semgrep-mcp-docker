...
readonly DEFAULT_PORT=7055
readonly DEFAULT_INTERNAL_PORT=37055
...
readonly HAPROXY_SERVER_NAME="semgrep-mcp"
...
handle_first_run() {
    ...
    # Alpine base doesn't have a 'semgrep' user, so create one
    if ! id -u semgrep >/dev/null 2>&1; then
        addgroup -g "$PGID" semgrep 2>/dev/null || true
        adduser -D -u "$PUID" -G semgrep -s /bin/bash semgrep 2>/dev/null || true
    fi
    ...
    if id -u semgrep >/dev/null 2>&1; then
        if [ "$(id -u semgrep)" -ne "$PUID" ]; then
            if usermod -o -u "$PUID" semgrep 2>/dev/null; then
                uid_gid_changed=1
            else
                PUID="$(id -u semgrep)"
            fi
        ...
        if [ "$(id -g semgrep)" -ne "$PGID" ]; then
            if groupmod -o -g "$PGID" semgrep 2>/dev/null; then
                uid_gid_changed=1
            else
                PGID="$(id -g semgrep)"
            fi
    ...
    # Ensure semgrep cache dirs are writable by container user
    mkdir -p /home/semgrep/.semgrep /home/semgrep/.cache/semgrep /opt/custom-rules
    chown -R "$PUID:$PGID" /home/semgrep /opt/custom-rules 2>/dev/null || true
...
generate_haproxy_config() {
    ...
    http-request track-sc0 src table rate_limit_table"
        ...
        if [[ "$RATE_LIMIT" != "0" ]]; then
            # Retry-After header expects seconds — convert RATE_LIMIT_PERIOD units
            local rl_num="${RATE_LIMIT_PERIOD%%[smhd]*}"
            local rl_unit="${RATE_LIMIT_PERIOD: -1}"
            local rl_seconds
            case "$rl_unit" in
                s) rl_seconds="$rl_num" ;;
                m) rl_seconds=$(( rl_num * 60 )) ;;
                h) rl_seconds=$(( rl_num * 3600 )) ;;
                d) rl_seconds=$(( rl_num * 86400 )) ;;
                *) rl_seconds="$rl_num" ;;
            esac
            rate_limit_check+="
    http-request return status 429 content-type \"application/json\" string '{\"error\":\"Too Many Requests\",\"message\":\"Rate limit exceeded\"}' hdr \"Retry-After\" \"${rl_seconds}\" if !is_health_check { sc_http_req_rate(0,rate_limit_table) gt ${RATE_LIMIT} }"
            echo "Rate limiting enabled: ${RATE_LIMIT} requests per ${RATE_LIMIT_PERIOD} (Retry-After: ${rl_seconds}s)"
        fi
...
start_mcp_server() {
    # Forward upstream-recognized env vars to the subprocess. Upstream semgrep
    # mcp reads these directly: SEMGREP_APP_TOKEN (auth), SEMGREP_MCP (marker),
    # USE_SEMGREP_RPC (backend toggle), plus 8 per-tool *_DISABLED toggles.
    # SEMGREP_RULES / SEMGREP_METRICS / SEMGREP_SEND_METRICS are standard
    # semgrep env vars, inherited by scan subprocesses.
    local forwarded_vars=(
        SEMGREP_APP_TOKEN SEMGREP_MCP USE_SEMGREP_RPC
        SEMGREP_RULES SEMGREP_METRICS SEMGREP_SEND_METRICS
        SEMGREP_RULE_SCHEMA_DISABLED
        GET_SUPPORTED_LANGUAGES_DISABLED
        SEMGREP_FINDINGS_DISABLED
        SEMGREP_SCAN_WITH_CUSTOM_RULE_DISABLED
        SEMGREP_SCAN_DISABLED
        SEMGREP_SCAN_REMOTE_DISABLED
        GET_ABSTRACT_SYNTAX_TREE_DISABLED
        SEMGREP_SCAN_SUPPLY_CHAIN_DISABLED
        PYTHONUNBUFFERED
    )
    local v
...
main() {
    ...
    # Capture installed semgrep version for banner
    SEMGREP_VERSION="$(semgrep --version 2>/dev/null | head -1 || echo unknown)"
    export SEMGREP_VERSION

    export PORT PUID PGID PROTOCOL PRO_MODE_DISPLAY SEMGREP_VERSION
